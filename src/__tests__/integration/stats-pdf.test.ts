/**
 * Tests d'intégration — POST /api/stats/pdf.
 *
...
import { POST } from '@/app/api/stats/pdf/route';
...
describe('POST /api/stats/pdf', () => {
    ...
    it('génère et retourne directement le PDF (happy path)', async () => {
        mockedAuth.mockResolvedValue({ user: { email: 'user@test.com', roles: ['CHVL'] } } as never);
        const res = await POST(makePostRequest({ dateFrom: '2026-01-01', dateTo: '2026-01-31' }));
        expect(res.status).toBe(200);
        expect(res.headers.get('Content-Type')).toBe('application/pdf');
        expect(res.headers.get('Content-Disposition')).toContain('stats-martine.pdf');
