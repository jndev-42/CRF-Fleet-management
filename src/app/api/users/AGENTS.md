...
- `POST` always inserts the `UserUL` row with `is_home = 1`, so it can never produce a non-conforming row (INACTIF outside the home UL). Conformance by construction, pinned by a test rather than left accidental
