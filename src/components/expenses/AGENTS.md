...
| `YousignSignatureModal.tsx` | Signature capture with two modes: `typed` (name rendered as a script signature) and `draw` (touch/mouse `<canvas>`, scaled by `getBoundingClientRect`). Collects the signer's function title and consent, then hands back a `SignatureData` (mode, base64 image, name, date, hash). Exports `SignatureData`. |
...
`ExpenseForm` is create *and* edit — the optional `initialData` prop seeds every field and switches the target endpoint. `YousignSignatureModal` resets its internal state on the `isOpen` false→true transition via a render-time `prevIsOpen` comparison, not a `useEffect`.
