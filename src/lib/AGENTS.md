...
| `roles.ts` | Role hierarchy/permission helpers. **Every authorization predicate is wrapped in `denyWhenInactive`**: an account carrying INACTIF (or the legacy GUEST) gets `false` everywhere, so the block reaches the ~47 API routes whose guards call `isAdminOrAbove`/`canAccessAdminPanel`/`isSuperAdmin` and never `isInactive` — `/api` is outside the middleware matcher (`src/proxy.ts:8`). A new predicate added without the wrapper fails the enumeration test in `__tests__/unit/roles.test.ts`. `isInactive` / `isQrBlocked` are **dominant**: carrying INACTIF (or the legacy GUEST) is enough, even alongside active roles. They diverge on one point only, deliberately — the empty role list is denied by `isInactive`, allowed by `isQrBlocked` |
