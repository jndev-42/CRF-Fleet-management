...
def get_repository_map():
    ...
    repo_map = []
    # On se concentre sur les dossiers de dev critiques
    ...
    for base_folder in target_roots:
        ...
        for root, dirs, files in os.walk(base_folder):
            ...
            # Détection de ton fichier de contexte (sensible à la casse)
            has_claude_md = "CLAUDE.md" in files or "claude.md" in files
            
            # On ne garde que les fichiers sources pertinents pour l'indexation légère
...
def ask_claude_for_scope(issue_title, issue_body, repo_map):
    ...
    try:
        ...
        print(f"Erreur de parsing du scope. Retour à la racine. Réponse brute : {response}")
...
def get_scoped_codebase_context(target_directories):
    ...
    # 1. On charge TOUS les CLAUDE.md trouvés dans le projet (Léger et crucial pour les consignes)
    ...
    # 2. On injecte le code source complet des répertoires ciblés par le triage
    ...
    for folder in target_directories:
        ...
        for file in os.listdir(folder):
            ...
            # Support du TypeScript, TSX et modules CSS de ton projet
...
def create_pull_request(branch_name, title, body):
    ...
    # Installation des dépendances si nécessaire dans le runner (optionnel selon ton workflow)
    # os.system("npm ci")
    
    # Exécution de Vitest en mode run (CI)
...
def execute_batch_request(model, system_prompt, user_prompt):
                    ...
                    "system": [
                        {
                            "type": "text",
                            "text": system_prompt,
                            "cache_control": {"type": "ephemeral"} # Cache ton pavé de code
                        }
                    ],
                    "messages": [{"role": "user", "content": user_prompt}]
                }
    ...
    for result in anthropic_client.beta.messages.batches.results(batch_job.id):
        if result.result.type == "succeeded":
            ...
            raise Exception("L'exécution du Batch Anthropic a échoué.")
...
# 2. CAS : ANALYZE (Batch API + Sonnet)
...
# 3. CAS : FEATURE (Interactif / No Batch + Opus puis Sonnet)
elif "feature" in labels:
    comments = issue.get_comments()
    discussion = "\n".join([f"{c.user.login}: {c.body}" for c in comments])

    # Phase d'architecture avec Opus
    ...
    if "[NEED_CLARIFICATION]" in opus_response:
        ...
        sys.exit(0) # On s'arrête en attendant la réponse de l'humain
    else:
        # Phase de dev avec Sonnet
